...
class Authentication with ChangeNotifier {
  ...
  Future<int> get getPostsNo async {
    ...
    await _cloudInstance.collection('posts').get();
  ...
  Future<bool> SignUp_user(String Email, String password, context, Fname, Lname, Age, Phone,type,gender) async {
    bool success = false;
    ...
    try {
      ...
      firebaseStorageRef
        ...
        taskSnapshot.ref.getDownloadURL().then((value) async {
          ...
          PhotoUrl = value;
          updateCollectionField(collectionName: 'users', fieldName: 'photoUrl', fieldValue: value, document_Id: userId);
  ...
  Future<String> uploadtoFireStorage({@required String  filePath ,@required File file}) async {
    String fileURL = '';
    try {
      print('Start upload Profile Image To Firebase');
      String userId = getUserId;
  ...
  Future<void> getCurrentUser() async {
    try {
      ...
      if (user != null) {
        ...
        print(user.uid);
        var userInfo= await _cloudInstance.collection('users').doc(user.uid).get();
  ...
  Future<void> AddNewPost(String title , String description,String eventType,String volunteersNo,String attendingNo,String date,String time,File file) async {
    try
    {
      ...
      String fileUrl = '';
      if ( file != null )
        fileUrl = await uploadtoFireStorage(filePath: getUserId+postNo.toString()+"/"+ getUserId+postNo.toString(), file: file);
      print('777777777777777777777777777');

      print("fileUrl from add post $fileUrl");
      var posts= _cloudInstance
          .collection('posts')
          .doc();
      posts.set({
        'title': title,
  ...
  Future<void> updateCollectionField ({@required String collectionName , @required String fieldName ,@required dynamic fieldValue,@required String document_Id  }) async{

    try {

      _cloudInstance.collection(collectionName).doc(document_Id).update({fieldName:fieldValue});
    } catch (e) {
      throw e;
    }
  }
